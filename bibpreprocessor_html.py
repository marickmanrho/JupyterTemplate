...
try:
    del unicode_tex.unicode_to_tex_map[" "]
except KeyError:
    pass
...
class BibTexPreprocessor(Preprocessor):
    ...
    def create_bibentry(self, refkey, reference):
        ...
        if reference["type"] == "article-journal":
            entry = "@article{" + str(refkey) + ",\n"
        elif reference["type"] == "book":
            entry = "@book{" + str(refkey) + ",\n"
        elif reference["type"] == "chapter":
            entry = "@inbook{" + str(refkey) + ",\n"
        else:
            # default type is misc!
            entry = "@misc{" + str(refkey) + ",\n"
        ...
        if "author" in reference:
            ...
            entry += " and ".join(
                map(
                    lambda a: unicode_tex.unicode_to_tex(a["family"])
                    + ", "
                    + unicode_tex.unicode_to_tex(a["given"]),
                    reference["author"],
                )
            )
            entry += "}, \n"
        ...
        if "title" in reference:
            if reference["type"] == "chapter":
                entry += (
                    "  chapter = {"
                    + unicode_tex.unicode_to_tex(reference["title"])
                    + "}, \n"
                )
            else:
                entry += (
        ...
        if "container-title" in reference:
            if reference["type"] == "chapter":
                entry += (
                    "  title = {"
                    + unicode_tex.unicode_to_tex(reference["container-title"])
                    + "}, \n"
                )
            else:
                entry += (
        ...
        if "publisher" in reference:
            entry += "  publisher = {" + str(reference["publisher"]) + "}, \n"
        if "page" in reference:
            entry += "  pages = {" + str(re.sub("-", "--", reference["page"])) + "}, \n"
        if "volume" in reference:
            entry += "  volume = {" + str(reference["volume"]) + "}, \n"
        if "issue" in reference:
            entry += "  issue = {" + str(reference["issue"]) + "}, \n"
        if "DOI" in reference:
            entry += "  doi = {" + str(reference["DOI"]) + "}, \n"
        if "URL" in reference:
            entry += "  url = {" + str(reference["URL"]) + "}, \n"
        ...
        if cell.cell_type == "markdown":
            ...
            if "html" in resources["output_extension"]:
                ...
                for citation in re.finditer(
                    '<cite data-cite=\\"(.*?)\\"></cite>', cell.source
                ):
                    tempcite = citeproc.Citation(
                        [citeproc.CitationItem(citation.groups()[0])]
                    )
                    self.bibliography.register(tempcite)
                    replaced = re.sub(
                        '<cite data-cite=\\"' + citation.groups()[0] + '\\"></cite>',
                        '<a href="#'
                        + tempcite["cites"][0]["key"]
                        + '">'
                        + str(self.bibliography.cite(tempcite, cite_warn))
                        + "</a>",
                        replaced,
                    )
            if '<div class="cite2c-biblio"></div>' in cell.source:
                if "html" in resources["output_extension"]:
                    ...
                    for item, key in zip(
                        self.bibliography.bibliography(), self.bibliography.keys
                    ):
